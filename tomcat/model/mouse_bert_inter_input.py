...
class InterMouseFeatureEmbeddings(nn.Module):
    def __init__(self, hidden_size, segment_embedding) -> None:
        super(InterMouseFeatureEmbeddings, self).__init__()
        self.hidden_size = hidden_size
        self.inter_mouse_feature_embed = nn.Linear(13, hidden_size)
        self.segment_embedding = segment_embedding

    def flatten(self, embeds, default=0):
        mice = len(embeds)
        N, length, dim, device = *embeds[0].shape, embeds[0].device
        new_tensor = torch.full((N, mice * length + 4, dim), fill_value=default, device=device)
        pos = 1
        for m in range(mice):
            start = pos
            end = pos + length
            new_tensor[:, start:end] = embeds[m]
            pos += length + 1

        return new_tensor

    def forward(self, inter_mouse_features):
        N, seq_len, features, device = *inter_mouse_features[0][1].shape, inter_mouse_features[0][1].device
        segment_embeddings = [self.segment_embedding(torch.full((N, seq_len), i, device=device)) for i in range(NUM_MICE)]

        embeds = {}
        for m1 in range(NUM_MICE):
            embeds[m1] = torch.zeros((N, seq_len, self.hidden_size), device=device)
...
class MouseBERTEmbeddings(nn.Module):
    '''
        Embed inputs
    '''

    def __init__(self, bert_config: RobertaConfig = None, num_features: int = None) -> None:
        
        super(MouseBERTEmbeddings, self).__init__()

        self.points_embedding = nn.Linear(num_features, bert_config.hidden_size)
        ...
        self.segment_embedding = nn.Embedding(3, bert_config.hidden_size)
        self.inter_mouse_embedding = InterMouseFeatureEmbeddings(bert_config.hidden_size, self.segment_embedding)
    
    def forward(self, N, seq_len, keypoints, masked_positions, segments, inter_mouse_features):

        # Create special token embeddings, not keen on this...
        zeros = torch.zeros((N, 1), dtype=torch.long, device=keypoints.device)
        ...
        # Embed inter mouse features
        points_embed += self.inter_mouse_embedding(inter_mouse_features)
        
        # Apply masking
        points_embed = masked_positions * mask_embed + ~masked_positions * points_embed
        segment_embed = self.segment_embedding(segments)

        embed = points_embed + segment_embed
...
class MouseBERTSep(nn.Module):
    ...
    def __init__(self, bert_config: RobertaConfig = None, num_features: int = None) -> None:
        ...
        # Backbone
        self.embeds = MouseBERTEmbeddings(bert_config, self.num_features)
        self.transformer = RobertaModel(bert_config)
        self.output_projection = nn.Linear(bert_config.hidden_size, self.output_size)
        self.pooler = Pooler(bert_config.output_size)
        ...
        self.task_head = RegressionHead(self.output_size, 53, dropout=0.2)
        self.inter_mouse_task_head = RegressionHead(self.output_size, 13, dropout=0.2)
        self.group_task_head = RegressionHead(bert_config.output_size, 1, dropout=0.2)
    ...
    def create_inter_mouse_embeddings(self, embeddings) -> torch.tensor:
        ...
        '''
        # First remove the special tokens and separate the embeddings
        pairs = {}
